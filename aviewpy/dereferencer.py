...
class Dereferencer():
            ...
            for ent in manager.values():
                ...
                for prop_name in ent.properties:
                    ...
                    if isinstance(prop_val, Object) and prop_val == self.entity:
                        ...
                        with adams_errors_suppressed():
                            try:
                                setattr(ent, prop_name, [v for v in prop_val if v != self.entity] or [
                                        self.entity_copy])
                            except SetValueFailed:
                                pass
    ...
    def _write_all_contact_incidents(self):
        ...
        for ans_name in set(ans_names):
            ...
            if False:
                ...
                file = self.export_dir / f'{ans.full_name}.res'
    ...
    def rereference(self):
        ...
        for file in self.files:
            if file.suffix == '.bin':
                Adams.execute_cmd(
                    f'file bin read file="{str(file)}" entity_name={file.stem} alert=no')

            elif file.suffix == '.res':
                Adams.execute_cmd(f'file analysis read file="{str(file)}" analysis="{file.stem}"')
